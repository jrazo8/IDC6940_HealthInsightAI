...
```
This table categorizes and summarizes all explanatory variables in the dataset.
...
table_data <- data.frame(
  Type = c("Target", rep("Binary", 14), rep("Ordinal", 6), "Continuous"),
  Variable = c("Diabetes_binary", "HighBP", "HighChol", "CholCheck", "Smoker", 
               "Stroke", "HeartDiseaseorAttack", "PhysActivity", "Fruits", "Veggies", 
               "HvyAlcoholConsump", "AnyHealthcare", "NoDocbcCost", "DiffWalk", "Sex",
               "GenHlth", "MentHlth", "PhysHlth", "Age", "Education", "Income", "BMI"),
  Description = c("Indicates whether a person has diabetes", 
  ...
  Range = c("(0 = No, 1 = Yes)", "(0 = No, 1 = Yes)", "(0 = No, 1 = Yes)", "(0 = No, 1 = Yes)", 
            "(0 = No, 1 = Yes)", "(0 = No, 1 = Yes)", "(0 = No, 1 = Yes)", "(0 = No, 1 = Yes)", 
            "(0 = No, 1 = Yes)", "(0 = No, 1 = Yes)", "(0 = No, 1 = Yes)", "(0 = No, 1 = Yes)", 
            "(0 = No, 1 = Yes)", "(0 = No, 1 = Yes)", "(0 = Female, 1 = Male)",
...
*Age, General Health (GenHlth)*, and *DiffWalk* show the strongest correlation with Diabetes_binary, suggesting these are significant predictors of diabetes.

*BMI* has a moderate correlation with *Diabetes_binary*, indicating it plays a role but is not the strongest predictor.

*HighBP* and *HighChol* still show some correlation with diabetes, but their impact appears weaker than before.

*GenHlth, MentHlth*, and *PhysHlth* have strong intercorrelations, which may indicate redundancy in the dataset.

*Education* and *Income* exhibit mild correlation with *Age*, which may suggest an indirect influence on health outcomes.
...
if target_variable:
    plt.figure(figsize=(8, 5))
    sns.boxplot(x=cdc_data_df[target_variable], y=cdc_data_df["BMI"], palette="Set3")
...
## Histograms the distribution of values

```{python}
import matplotlib.pyplot as plt

# Ensure the dataframe is loaded correctly
if 'cdc_data_df' in globals():
    df = cdc_data_df  # Assign the dataset
else:
    raise ValueError("Dataset 'cdc_data_df' is not defined. Please check your variable name.")

# Number of columns in the dataset
num_columns = len(df.columns)

# Calculate the number of rows needed (3 histograms per row)
...
for i, col in enumerate(df.columns):
    if df[col].dtype in ['int64', 'float64']:  # Ensure only numeric columns are plotted
        axes[i].hist(df[col], bins=20, color='seagreen', edgecolor='black')
        axes[i].set_title(col, fontsize=10)
        axes[i].set_xlabel('Values')
        axes[i].set_ylabel('Frequency')
    else:
        axes[i].remove()  # Remove plot if column is not numerical
...
## Income vs Diabetes Status Plot
...
# Ensure Diabetes_binary column is of integer type for proper hue mapping
cdc_data_df['Diabetes_binary'] = cdc_data_df['Diabetes_binary'].astype(int)

# Define color mapping for diabetes status (as integers)
palette_colors = {0: "mediumaquamarine", 1: "gold"}

# Create the countplot
plt.figure(figsize=(10, 5))
sns.countplot(x='Income', hue='Diabetes_binary', data=cdc_data_df, palette=palette_colors)

# Add labels and title
plt.title('Income vs Diabetes Status', fontsize=16)
...
The analysis explored the relationship between income levels and diabetes prevalence. 

The countplot indicates that the majority of individuals belong to higher income categories and are non-diabetic.

Diabetes cases, although present, remain consistently lower across all income levels. 

While there appears to be a socioeconomic factor, further statistical testing is required to determine the strength of the association.

## Education vs Diabetes Status Plot

```{python}

# Define color palette for diabetes status
palette_colors = {0: "mediumaquamarine", 1: "gold"}

# Create the count plot
plt.figure(figsize=(10, 5))
sns.countplot(x="Education", hue="Diabetes_binary", data=cdc_data_df, palette=palette_colors)

# Add labels and title
plt.title("Education vs Diabetes Status", fontsize=16)
...
The visualization of Education vs. Diabetes Status suggests that individuals with lower education levels may have a slightly higher proportion of diabetes cases. However, since the total number of individuals per category varies significantly, further statistical tests may be needed to confirm any correlation.


## General Health vs Diabetes Status Plot
...
# Define colors explicitly for diabetes status
palette_colors = {0: "mediumaquamarine", 1: "gold"}
...
## BMI Density Plot by Diabetes Status
