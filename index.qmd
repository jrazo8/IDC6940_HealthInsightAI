...
# Create a dataset for animation
df <- data.frame(
  x = c(2, 4, 6, 8, 10),
  y = c(3, 5, 7, 9, 11),
  class = factor(c("A", "B", "A", "B", "A")),
  step = c(1, 2, 2, 3, 3) # Different steps for animation
)

# Create an animated classification plot
p <- ggplot(df, aes(x, y, color = class)) +
  geom_point(size = 5) +
  labs(title = "Step: {closest_state}") +
  transition_states(step, transition_length = 2, state_length = 1)

# Save or render the animation
animate(p, renderer = gifski_renderer("classification_steps.gif"))
